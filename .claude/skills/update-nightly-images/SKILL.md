...
2. Look up the latest stable release of each:
   ...
   - **Each PECL extension** (currently just `redis`): `https://pecl.php.net/rest/r/<name>/allreleases.xml` — take the newest release whose `<s>` is `stable`, or check https://pecl.php.net/package/<name> ("Latest Stable Release").
